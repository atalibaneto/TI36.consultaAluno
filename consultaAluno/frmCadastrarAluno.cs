        ...
        private void SalvarAluno()
        {
            try
            {
                using (SqlConnection cn = new SqlConnection (conexao.IniciarCon)) //cria uma nova conexão com o banco
                {
                    ...
                    if (this.idAluno == 0)
                        sql = "INSERT INTO alunos (nomeAluno, dataNascAluno, emailAluno, telefoneAluno, anoAluno)" +
                        "VALUES (@nome, @data, @email, @tel, @ano)";
                    else
                        sql = "UPDATE alunos SET nomeAluno=@nome, dataNascAluno=@data, emailAluno=@email, telefoneAluno=@tel, anoAluno=@ano WHERE idAluno=" + this.idAluno;
